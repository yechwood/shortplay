...
public class MainActivity extends AppCompatActivity {
    ...
    void openViewer(int index){
        ...
        viewerPlayer=new PlayerView(this);viewerPlayer.setUseController(true);viewerPlayer.setControllerAutoShow(true);viewerPlayer.setControllerHideOnTouch(true);viewerPlayer.setControllerShowTimeoutMs(2500);viewerPlayer.setResizeMode(androidx.media3.ui.AspectRatioFrameLayout.RESIZE_MODE_FIT);viewerPlayer.setVisibility(View.GONE);viewerPlayer.setOnTouchListener(new View.OnTouchListener(){float downX;public boolean onTouch(View v,MotionEvent e){if(e.getActionMasked()==MotionEvent.ACTION_DOWN){downX=e.getX();}else if(e.getActionMasked()==MotionEvent.ACTION_UP){float dx=e.getX()-downX;if(Math.abs(dx)>dp(70)){int n=dx<0?viewerIndex+1:viewerIndex-1;if(n>=0&&n<media.size())animateViewerTo(n,dx<0);return true;}}return false;}});box.addView(viewerPlayer,new FrameLayout.LayoutParams(-1,-1));
