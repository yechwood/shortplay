...
public class MainActivity extends AppCompatActivity {
    ...
    int viewerIndex=-1;float videoDownX;
    ...
    int savedFirst=-1,savedOffset=0; View viewerSourceView; boolean openingViewer=false;
    ...
    void loadClips(){
        ...
        String[] permissions;if(Build.VERSION.SDK_INT>=34)permissions=new String[]{Manifest.permission.READ_MEDIA_IMAGES,Manifest.permission.READ_MEDIA_VIDEO,Manifest.permission.READ_MEDIA_VISUAL_USER_SELECTED};else if(Build.VERSION.SDK_INT>=33)permissions=new String[]{Manifest.permission.READ_MEDIA_IMAGES,Manifest.permission.READ_MEDIA_VIDEO};else permissions=new String[]{Manifest.permission.READ_EXTERNAL_STORAGE};
        ...
        try(Cursor c=getContentResolver().query(iu,ic,null,null,MediaStore.Images.Media.DATE_MODIFIED+" DESC")){
            if(c!=null)while(c.moveToNext()){String f=mediaFolder(c.getString(3),null);MediaItemData x=new MediaItemData(ContentUris.withAppendedId(iu,c.getLong(0)),c.getString(1),false,0,c.getLong(2),f,c.getInt(4),c.getInt(5));media.add(x);folders.add(f);}
        }catch(Exception ignored){}
        sortMedia();
    }
    String cleanFolder(String s){if(s==null||s.trim().isEmpty())return "Other";s=s.replace("\\\\","/");while(s.endsWith("/"))s=s.substring(0,s.length()-1);return s.trim();}
    String mediaFolder(String relative,String bucket){if(relative!=null&&!relative.trim().isEmpty())return cleanFolder(relative);if(bucket!=null&&!bucket.trim().isEmpty())return cleanFolder(bucket);return "Other";}
    void sortMedia(){
    ...
    void buildVisible(){
        ...
        else for(MediaItemData y:media) if(cleanFolder(y.folder).equals(cleanFolder(currentFolder))&&((y.video&&showVideos)||(!y.video&&showPhotos))) visible.add(y);
    }
    void render(){
        buildVisible();
    ...
    class GalleryAdapter extends RecyclerView.Adapter<GalleryAdapter.VH>{
        class VH extends RecyclerView.ViewHolder{ImageView thumb;TextView badge;VH(View v,ImageView i,TextView b){super(v);thumb=i;badge=b;}}
        public VH onCreateViewHolder(ViewGroup p,int t){int w=getResources().getConfiguration().screenWidthDp;int h=w<=360?160:150;LinearLayout card=new LinearLayout(MainActivity.this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(3,3,3,3);card.setBackground(rounded(themeCard(),14));FrameLayout fr=new FrameLayout(MainActivity.this);ImageView im=new ImageView(MainActivity.this);im.setScaleType(ImageView.ScaleType.CENTER_CROP);fr.addView(im,new FrameLayout.LayoutParams(-1,dp(138)));TextView b=text("",18);b.setGravity(Gravity.CENTER);b.setTextColor(Color.WHITE);b.setBackground(rounded(Color.argb(170,0,0,0),20));FrameLayout.LayoutParams bp=new FrameLayout.LayoutParams(dp(38),dp(38),Gravity.BOTTOM|Gravity.END);bp.setMargins(0,0,dp(7),dp(7));fr.addView(b,bp);card.addView(fr);RecyclerView.LayoutParams rp=new RecyclerView.LayoutParams(-1,dp(h));rp.setMargins(3,3,3,5);card.setLayoutParams(rp);return new VH(card,im,b);}
        public void onBindViewHolder(VH h,int p){MediaItemData x=visible.get(p);h.badge.setText("▶");h.badge.setVisibility(x.video?View.VISIBLE:View.GONE);h.itemView.setAlpha(selectionMode&&selected.contains(x)?0.55f:1f);h.thumb.setTag(x.uri);loadThumbnail(x,h.thumb);ViewCompat.setTransitionName(h.thumb,"media_"+x.uri.toString());h.itemView.setOnClickListener(v->{if(selectionMode){if(selected.contains(x))selected.remove(x);else selected.add(x);h.itemView.setAlpha(selected.contains(x)?0.55f:1f);}else openViewer(media.indexOf(x),h.thumb);});h.itemView.setOnLongClickListener(v->{if(!selectionMode){selectionMode=true;selected.clear();selected.add(x);showHome();}return true;});}
        public int getItemCount(){return visible.size();}
    }
    void loadThumbnail(MediaItemData x,ImageView target){Uri u=x.uri;new Thread(()->{Bitmap b=null;try{if(Build.VERSION.SDK_INT>=29)b=getContentResolver().loadThumbnail(u,new android.util.Size(dp(900),dp(900)),null);else if(x.video){MediaMetadataRetriever r=new MediaMetadataRetriever();r.setDataSource(this,u);b=r.getFrameAtTime(0,MediaMetadataRetriever.OPTION_CLOSEST_SYNC);r.release();}else{try(InputStream in=getContentResolver().openInputStream(u)){b=BitmapFactory.decodeStream(in);}}}catch(Exception ignored){}Bitmap z=b;runOnUiThread(()->{if(z!=null&&u.equals(target.getTag()))target.setImageBitmap(z);});}).start();}

    void openViewer(int index){openViewer(index,null);}
    void openViewer(int index,View sourceView){
        savedPosition();
        viewerIndex=index;viewerSourceView=sourceView;openingViewer=sourceView!=null;viewerDialog=new Dialog(this,R.style.ViewerTheme);
        ...
        viewerPlayer=new PlayerView(this);viewerPlayer.setUseController(true);viewerPlayer.setControllerAutoShow(true);viewerPlayer.setControllerHideOnTouch(true);viewerPlayer.setControllerShowTimeoutMs(2500);viewerPlayer.setResizeMode(androidx.media3.ui.AspectRatioFrameLayout.RESIZE_MODE_FIT);viewerPlayer.setVisibility(View.GONE);viewerPlayer.setOnTouchListener((v,e)->{if(e.getActionMasked()==MotionEvent.ACTION_DOWN)videoDownX=e.getX();else if(e.getActionMasked()==MotionEvent.ACTION_UP){float dx=e.getX()-videoDownX;if(Math.abs(dx)>dp(70)){int n=dx<0?viewerIndex+1:viewerIndex-1;if(n>=0&&n<media.size())animateViewerTo(n,dx<0);return true;}}return false;});box.addView(viewerPlayer,new FrameLayout.LayoutParams(-1,-1));
        ...
        Button back=smallButton("‹");viewerTitle=text("",13);viewerTitle.setGravity(Gravity.CENTER);viewerTitle.setSingleLine(true);Button more=smallButton("⋮");top.addView(back,new LinearLayout.LayoutParams(dp(52),dp(48)));Space spacer=new Space(this);top.addView(spacer,new LinearLayout.LayoutParams(0,dp(48),1));top.addView(more,new LinearLayout.LayoutParams(dp(52),dp(42)));box.addView(top,new FrameLayout.LayoutParams(-1,dp(74),Gravity.TOP));
        back.setOnClickListener(v->closeViewerAnimated(viewerSourceView));more.setOnClickListener(v->{if(viewerIndex>=0)showMediaMenu(media.get(viewerIndex));});
        viewerDialog.setContentView(box);viewerDialog.setOnDismissListener(v->{releaseViewerPlayer();restorePosition();});viewerDialog.setOnKeyListener((d,key,event)->{if(key==android.view.KeyEvent.KEYCODE_BACK&&event.getAction()==android.view.KeyEvent.ACTION_UP){closeViewerAnimated();return true;}return false;});viewerDialog.setOnShowListener(v->{immersive(viewerDialog);box.setAlpha(0f);box.animate().alpha(1f).setDuration(160).start();});viewerDialog.show();immersive(viewerDialog);showViewerItem(index);
    }
    undefined
    undefined
    void animateViewerFromGrid(View source){if(viewerImage==null||source==null)return;source.post(()->{if(viewerImage.getWidth()==0||viewerImage.getHeight()==0){viewerImage.postDelayed(()->animateViewerFromGrid(source),30);return;}int[] a=new int[2],b=new int[2];source.getLocationOnScreen(a);viewerImage.getLocationOnScreen(b);float tx=a[0]+source.getWidth()/2f-(b[0]+viewerImage.getWidth()/2f);float ty=a[1]+source.getHeight()/2f-(b[1]+viewerImage.getHeight()/2f);float sx=Math.max(.08f,source.getWidth()/(float)Math.max(1,viewerImage.getWidth()));float sy=Math.max(.08f,source.getHeight()/(float)Math.max(1,viewerImage.getHeight()));viewerImage.setPivotX(viewerImage.getWidth()/2f);viewerImage.setPivotY(viewerImage.getHeight()/2f);viewerImage.setTranslationX(tx);viewerImage.setTranslationY(ty);viewerImage.setScaleX(sx);viewerImage.setScaleY(sy);viewerImage.setAlpha(.9f);viewerImage.animate().translationX(0).translationY(0).scaleX(1f).scaleY(1f).alpha(1f).setDuration(360).setInterpolator(new android.view.animation.DecelerateInterpolator()).start();});}
    void closeViewerAnimated(View source){if(viewerDialog==null||!viewerDialog.isShowing())return;if(source==null){closeViewerAnimated();return;}if(viewerImage==null){viewerDialog.dismiss();return;}int[] a=new int[2],b=new int[2];source.getLocationOnScreen(a);viewerImage.getLocationOnScreen(b);float tx=a[0]+source.getWidth()/2f-(b[0]+viewerImage.getWidth()/2f);float ty=a[1]+source.getHeight()/2f-(b[1]+viewerImage.getHeight()/2f);float sx=Math.max(.08f,source.getWidth()/(float)Math.max(1,viewerImage.getWidth()));float sy=Math.max(.08f,source.getHeight()/(float)Math.max(1,viewerImage.getHeight()));viewerImage.animate().translationX(tx).translationY(ty).scaleX(sx).scaleY(sy).alpha(.15f).setDuration(300).setInterpolator(new android.view.animation.AccelerateInterpolator()).withEndAction(()->viewerDialog.dismiss()).start();}
